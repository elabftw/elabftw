...
on('export-to', (el: HTMLElement) => {
  ...
  const linkedEntities = (document.getElementById(`${format}_exportWithLinks`) as HTMLInputElement).checked ? 1 : 0;
  ...
  window.open(`/api/v2/${el.dataset.type}/${el.dataset.id}?format=${finalFormat}&changelog=${changelog}&links=${linkedEntities}&json=${json}&classification=${classification}`, '_blank');
});
