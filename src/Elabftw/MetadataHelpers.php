...
final class MetadataHelpers
{
    ...
    public static function validateAndNormalizeMetadataValue(
        ...
        return self::normalizeMetadataValue($name, $field, $value, $preserveNumericTypes);
    }
    ...
    private static function normalizeMetadataValue(
        ...
        return self::normalizeMetadataScalarValue($name, $type, $value, $preserveNumericTypes);
    }
    ...
    private static function normalizeMetadataScalarValue(
        ...
        return match ($type) {
            ...
            'date' => self::normalizeDateValue($name, $value, false),
            'datetime-local' => self::normalizeDateValue($name, $value, true),
    ...
    /**
     * Normalize imported dates to the format expected by HTML date inputs.
     * Supports common CSV date formats, including DD/MM/YYYY.
     * Accepted formats include DD.MM.YYYY, DD-MM-YYYY, DD/MM/YYYY, YYYY-MM-DD and datetime variants
     */
    private static function normalizeDateValue(string $name, string $value, bool $withTime): string
    {
        if ($value === '') {
            return '';
        }

        $date = self::parseMetadataDate($value);
        if ($date === false) {
            throw new ImproperActionException(sprintf(_('Metadata field %s contains an invalid date.'), $name));
        }
        ...
        // Keep seconds when they contain information, otherwise match the value
        // generated by the datetime-local inputs in the UI
        $format = $date->format('s') === '00' ? 'Y-m-d\\TH:i' : 'Y-m-d\\TH:i:s';
