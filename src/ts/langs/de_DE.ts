/* eslint-disable quotes */
/* This file is generated by "bin/console dev:i18n4js" command, DO NOT EDIT directly */
const t = {
  translation: {
    "2FA": "2FA",
    "accessibility": "Barrierefreiheit",
    "action": "Aktion",
    "add": "Hinzufügen",
    "add-compound": "Verbindung hinzufügen",
    "add-task": "Aufgabe hinzufügen",
    "add-team": "Team hinzufügen",
    "add-to-team": "Ausgewählte Benutzer zum Team hinzufügen",
    "add-user-error": "Benutzer über Autovervollständigungsmenü hinzufügen.",
    "all-teams": "Alle Teams",
    "archive-user": "Nutzer archivieren",
    "archive-user-description": "Die Archivierung eines Nutzers deaktiviert seinen Zugang. Diese Aktion ist umkehrbar.",
    "association-date": "Datum der Zuordnung",
    "auto-size-columns": "Auto-size columns",
    "background-color": "Hintergrundfarbe",
    "back-to-top": "Nach oben",
    "can-manage-compounds": "Darf Substanzen verwalten",
    "can-manage-inventory-locations": "Darf Inventar-Standorte verwalten",
    "can-manage-users2teams": "Kann Nutzende Teams zuweisen",
    "cancel": "Abbrechen",
    "categories": "Kategorien",
    "category": "Kategorie",
    "clear": "Zurücksetzen",
    "click-to-edit": "Zum bearbeiten anklicken",
    "check-required": "Bitte überprüfen Sie die erforderlichen Felder.",
    "copied": "In Zwischenablage kopiert.",
    "comment-add": "Einen Kommentar hinzufügen",
    "confirm-clear-spreadsheet": "Aktuelle Tabelle verwerfen? Alle nicht gespeicherten Änderungen gehen verloren.",
    "custom-id": "Selbstgewählte ID",
    "custom-id-in-use": "Benutzerdefinierte ID wird bereits verwendet! Bitte andere verwenden.",
    "created-at": "Erstellt am",
    "created-on": "Erstellt am",
    "create-one-experiments": "Ein Experiment ohne Template erstellen",
    "create-one-items": "Eine Ressource ohne Template erstellen",
    "create-one-experiments_templates": "Ein Experiment-Template erstellen",
    "create-one-items_types": "Ein Ressourcen-Template erstellen",
    "create-resource-from-template": "Erstelle Ressource aus Vorlage",
    "create-experiment-from-template": "Erstelle Experiment aus Vorlage",
    "current-edit": "Wird gerade bearbeitet",
    "custom-fields": "Benutzerdefinierte Felder",
    "delete": "Löschen",
    "delete-success": "Erfolgreich gelöscht",
    "done": "Erledigt",
    "dropzone-upload-area": "Dateien zum Hochladen hier fallen lassen",
    "dropzone-filesize-limit": "Maximale Dateigröße:",
    "edit": "Bearbeiten",
    "edit-compound": "Verbindung bearbeiten",
    "editing-metadata": "Sie bearbeiten gerade die Metadaten zu diesem Eintrag.",
    "email-sent-to-x": "E-Mail wurde an {{num, number}} Benutzer gesendet.",
    "email": "E-Mail",
    "enable-permission": "Sie müssen mindestens eine Berechtigung vergeben.",
    "encryption": "Verschlüsselung",
    "entity-default-title": "Unbenannt",
    "entity-delete-warning": "Die ausgewählte(n) Ressource(n) und alle zugehörigen Daten werden dauerhaft gelöscht. Dies kann nicht rückgängig gemacht werden. Sind Sie sicher?",
    "entity-apply-both-permissions-warning": "Dadurch werden diese Berechtigungen sowohl auf die Lese- als auch auf die Schreibberechtigungen angewendet. Fortfahren?",
    "entity-patch-multi-warning": "Es wurden {{count, number}} Einträge geändert, aber {{failed, number}} Einträge konnten nicht geändert werden. Sie wurden rot markiert.",
    "entries-processed": "{{num, number}} Einträge verarbeitet",
    "error": "Fehler",
    "error-no-filename": "Dateiname fehlt.",
    "error-fetch-request": "Beim Abrufen der angeforderten Daten ist ein Fehler aufgetreten: {{error}}",
    "error-parsing-metadata": "Beim Parsen der Metadaten (benutzerdefinierte Felder) ist ein Fehler aufgetreten. Der vollständige Fehler wurde in der Browser-Konsole protokolliert.",
    "excellent-accessibility": "Hervorragende Barrierefreiheit",
    "existing-ror-associations": "Bestehende ROR-Zuordnungen",
    "experiment": "Experiment",
    "experiment-template": "Experimentvorlagen",
    "export": "Exportieren",
    "export-success": "Export erfolgreich.",
    "filename": "Dateiname",
    "firstname": "Vorname",
    "format": "Format",
    "good-accessibility": "Gute Barrierefreiheit",
    "id": "ID",
    "illegal-action": "Diese Aktion kann nicht auf die aktuelle Auswahl angewendet werden",
    "is-admin": "Ist Admin",
    "is-archived": "Ist archiviert",
    "is-owner": "Ist Eigentümer",
    "is-sysadmin": "Ist Sysadmin",
    "item": "Ressource",
    "item-type": "Ressourcenvorlage",
    "last-login": "Letzte Anmeldung",
    "lastname": "Nachname",
    "file-imported": "Datei erfolgreich importiert",
    "file-too-large": "Fehler: Datei ist zu groß!",
    "filter-by-category": "Nach Kategorie filtern",
    "form-validation-error": "Fehler bei der Validierung des Formulars.",
    "generic-delete-warning": "Element löschen?",
    "hide-deleted": "Gelöschte ausblenden",
    "filter-delete-warning": "Diesen Filter entfernen?",
    "fit-columns-to-table": "Fit columns to table",
    "import": "Import",
    "import-spreadsheet": "Importiere Tabelle (xls, csv, ods, ...)",
    "import-error": "Fehler beim Import. Bitte überprüfen Sie die Konsole für weitere Informationen.",
    "info-deleted-entries": "Sollen {{count, number}} {{entity, string}} wirklich gelöscht werden?",
    "info-deleted-entry": "Soll {{entry, string}} wirklich gelöscht werden?",
    "insufficent-contrast": "Unzureichender Kontrast",
    "invalid-info": "Ungültige Syntax oder Angaben.",
    "is-locked": "Ist gesperrt",
    "large-text-only": "Nur großer Text",
    "last-modified-at": "Zuletzt geändert am",
    "loading": "Laden",
    "link-delete-warning": "Diesen Link löschen?",
    "location-name": "Standortname",
    "modified-at": "Geändert am",
    "multi-changes-confirm": "Anwenden auf {{num, number}} Einträge?",
    "my-teams": "Meine Teams",
    "name": "Name",
    "new-spreadsheet": "Neue Tabelle",
    "new-version": "Eine neue Version ist verfügbar!",
    "next-step": "Nächster Schritt",
    "no-self-links": "Eine Ressource kann nicht mit sich selbst verknüpft werden. Bitte wählen Sie ein anderes Ziel.",
    "not-found": "Kein passendes Ergebnis gefunden.",
    "no-rors": "Noch keine ROR zugeordnet.",
    "not-set": "Nicht festgelegt",
    "no-tasks-yet": "Noch keine Aufgaben.",
    "nothing-selected": "Nichts ausgewählt!",
    "now": "Jetzt",
    "oc-version-warning": "Warnung: Die aktuelle OpenCloning-Version ist mit dieser Version von eLabFTW nicht kompatibel.",
    "onboarding-email-sent": "Onboarding-E-Mail(s) wird/werden in Kürze versendet.",
    "only-a-sysadmin": "Nur ein Sysadmin kann dies bearbeiten.",
    "other": "Sonstiges",
    "owner": "Besitzer",
    "ownership-transfer": "Ihr Eintrag wurde erfolgreich an den ausgewählten Benutzer übertragen.",
    "organisation-name": "Organisationsname",
    "please-wait": "Bitte warten…",
    "rating": "Bewertung",
    "read-release-notes": "Versionshinweise lesen",
    "readonly": "Schreibgeschützt",
    "remove": "Entfernen",
    "rename-column": "Neue Spaltenüberschrift",
    "replace-edited-file": "Möchten Sie die Datei auf dem Server durch diese Bearbeitung ersetzen?",
    "replace-existing": "Überschreibe Originaldatei",
    "request-filename": "Geben Sie den Namen der Datei ein",
    "reset": "Zurücksetzen",
    "resource-not-found": "Nichts zum Anzeigen mit dieser ID",
    "restore-default-layout": "Restore default layout",
    "revisions-error": "Wählen Sie zwei Revisionen aus, um sie zu vergleichen.",
    "ror-description": "Diese RORs gelten für die gesamte Instanz und werden an alle Teams vererbt.",
    "ror-description-team": "Diese RORs gelten nur für das aktuelle Team.",
    "ror-description-user": "Diese RORs gelten nur für Ihr Konto.",
    "ror-input-label": "Kennung des Research Organization Registry (ROR) hinzufügen",
    "ror-input-title": "Geben Sie eine gültige ROR-ID mit 9 Zeichen ein, zum Beispiel 04t0gwh46 oder https://ror.org/04t0gwh46",
    "save": "Speichern",
    "save-and-go-back": "Speichern und zurückgehen",
    "save-attachment": "Als Anhang speichern",
    "saved": "Gespeichert",
    "search": "Suche",
    "select-resource": "Ressource auswählen",
    "session-expiring": "Ihre Sitzung läuft aufgrund von Inaktivität bald ab.",
    "session-expiring-title": "eLabFTW-Sitzung läuft ab",
    "show-all": "Zeige alle",
    "show-archived": "Zeige archivierte",
    "show-deleted": "Gelöschte einblenden",
    "signed-by": "Unterschrieben von",
    "signing": "Unterzeichnen",
    "sort-by-column": "Nach Spalte sortieren",
    "started-on": "Begonnen am",
    "status": "Status",
    "step-delete-warning": "Diesen Schritt löschen?",
    "step-unfinish-warning": "Sind Sie sicher, dass Sie diesen Schritt rückgängig machen wollen?",
    "tag-delete-warning": "Diesen Tag löschen?",
    "tags": "Tags",
    "table-options": "Table options",
    "team": "Team",
    "teams": "Teams",
    "template-title": "Titel der Vorlage",
    "text-color": "Textfarbe",
    "is-timestamped": "Mit Zeitstempel versehen",
    "timestamped-by": "Mit Zeitstempel versehen von",
    "title": "Titel",
    "today": "Heute",
    "toggle-pin": "Pin umschalten",
    "type-3-chars": "Mindestens 3 Zeichen sind für die Suche erforderlich",
    "unarchive-and-add-to-team": "Entarchivieren und hinzufügen zu Team {{team, string}}",
    "undefined-group": "Unbestimmte Gruppe",
    "unarchive-user": "Benutzer entarchivieren",
    "unarchive-user-description": "De-Archivierung eines Benutzers stellt den Zugang wieder her. Dies ist umkehrbar.",
    "upload-file-comment": "Dateikommentar",
    "userid": "User-ID",
    "users": "Benutzer",
    "valid-until": "Gültig bis",
    "validated": "Bestätigt",
    "view-template": "Vorlage anschauen",
    "view-upgrade-guide": "Upgrade-Anleitung anzeigen",
  },
};
export default t;
