...
class UsersView
{
     ...
     * @param Users $users
     */
    public function __construct(Users $users, Auth $auth)
    {
        $this->Users = $users;
        $this->Auth = $auth;
    }
    /**
     * Display users edit form
     *
     * @return string
     */
    public function showEditUsers($team = null, $fromSysconfig = false)
    {
        if ($fromSysconfig) {
        ...
        foreach ($usersArr as $user) {
            ...
            $html .= "<input autocomplete='new-password' class='clean-form' id='usersUpdatePassword' type='password' pattern='.{0}|.{" .
                $this->Auth::MIN_PASSWORD_LENGTH . ",}' value='' name='password' />";
            $html .= "<span class='smallgray'>" .
                $this->Auth::MIN_PASSWORD_LENGTH . " " . _('characters minimum') . "</span></li>";
            $html .= "</ul>";
