describe('Status in admin panel', () => {
  ...
  it('Create, update and delete a status', () => {
    ...
    cy.get('[data-action="destroy-status"][data-target="experiments_status"]').then(allBtn => {
      ...
      cy.get(`[data-action="destroy-status"][data-target="experiments_status"][data-id="${id}"]`).click();
