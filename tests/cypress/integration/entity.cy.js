describe('Experiments', () => {
  ...
  const entityEdit = () => {
    ...
    cy.get('#date_input').type('2021-05-01').blur().wait(500)
    cy.get('#overlay').should('be.visible').should('contain', 'Save')
    cy.get('#title_input').type('Updated from cypress').blur().wait(500)
    cy.get('#overlay').should('be.visible').should('contain', 'Save')
    cy.get('#createTagInput').type('some tag').blur().wait(500)
    cy.get('#overlay').should('be.visible').should('contain', 'Save')
  ...
  it('Create and edit an experiment', () => {
    ...
    entityEdit()
    cy.get('#category_select').select('Success').blur().wait(500)
    cy.get('#overlay').should('be.visible').should('contain', 'Save')
    entityComment()
    entityDuplicate()
    entityDestroy()
