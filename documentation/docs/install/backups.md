...
It is necessary to use the `elabctl.conf` configuration file (available [here](https://raw.githubusercontent.com/elabftw/elabftw/refs/heads/master/containers/elabctl/elabctl.conf)). Place this file in `/root/.config/elabctl.conf` and make sure to specify the settings correctly.
