describe('Experiments', () => {
  ...
  const entityEdit = () => {
    ...
    cy.get('#date_input').type('2021-05-01').blur().wait(100)
    cy.contains('Saved').should('be.visible')
    cy.get('#title_input').type('Updated from cypress').blur().wait(100)
    cy.contains('Saved').should('be.visible')
    cy.get('#createTagInput').type('some tag').blur().wait(100)
    cy.contains('Saved').should('be.visible')
  ...
  it('Create and edit an experiment', () => {
    ...
    entityEdit()
    cy.get('#category_select').select('Success').blur().wait(100)
    cy.contains('Saved').should('be.visible')
    entityComment()
    entityDuplicate()
    entityDestroy()
