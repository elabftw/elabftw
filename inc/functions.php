...
function check_for_updates() {
    ...
    if (!check_executable('git')) {
        return "<a class='errors' href='#'>Install git to check for updates.</a>";
    }

    // check that curl extension is installed and loaded
    if (!extension_loaded('curl')) {
        return "<a class='errors' href='https://github.com/NicolasCARPi/elabftw/wiki/Troubleshooting#manual-intervention-needed'>You need to install the curl extension for php. Click this message.</a>";
    }
    // check if the proxy settings exists
    $ini_arr = parse_ini_file('admin/config.ini');
    if (!isset($ini_arr['proxy'])) {
        return "<a class='errors' href='https://github.com/NicolasCARPi/elabftw/wiki/Troubleshooting#proxy-setting-not-present'>Proxy setting not present. Click this message.</a>";
    }
    ...
    if (strlen($latest_version) != 40) { // couldn't connect
        return "<a class='errors' href='https://github.com/NicolasCARPi/elabftw/wiki/Troubleshooting#couldnt-connect-to-github-to-check-for-updates'>Couldn't connect to github.com to check for updates.</a> ";
    }
    if ($latest_version != $current_version) {
        return "<a class='errors' href='https://github.com/NicolasCARPi/elabftw#updating'>Update available !</a> ";
    } 
    // we return nothing if there is nothing to say
    return "";
}
?>
